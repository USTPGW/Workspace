...
double Cz(double u, double w, double i)
{return -4.176*(alpha(u,w)+0.875*i)*(1-(alpha(u,w)*alpha(u,w)/2)) - (0.010652+0.0393*(4.176*(alpha(u,w)+0.875*i)*(4.176*(alpha(u,w)+0.875*i))))*alpha(u,w);}
...
/*w-dot*/			out[2] = x[2] + dt*(qbar(x[0],x[1],x[2])*S*Cz(x[0],x[2],i)/mass + gravity*(1-(x[3]*x[3])/2)*(1-(x[4]*x[4])/2) - (netbuoyancy/mass)*(1-(x[3]*x[3])/2)*(1-(x[4]*x[4])/2) + x[7]*x[0] - x[6]*x[1]);
...
{ // observation update equation
	...
	out[5] = gravity*(1-(x[3]*x[3])/2)*(1-(x[4]*x[4])/2) + x[19]; // accelerometer z
...
void ut1(double *X, double *Wm, double *Wc, double uL, double *Q, double *x1, double *X1, double *P1, double *X2)
{
	...
	for(countrow=0;countrow<24;countrow++){
		for(countcol=0;countcol<49;countcol++){
			X2T[countcol*24+countrow] = X2[countrow*49+countcol]; // X2' (tranpose), 49x24
		}
	}
	
	for(countrow=0;countrow<24;countrow++){
		temp[countrow*49] = X2[countrow*49]*Wc[0];
		for(countcol=1;countcol<49;countcol++){
			temp[countrow*49 + countcol] = X2[countrow*49 + countcol]*Wc[countcol]; //X1*diag(Wc), output is 24x49
		}
	}
...
void ut2(double *X1, double *Wm, double *Wc, double m, double *R, double *z1, double *Z1, double *P2, double *Z2)
{
	...
	for(countrow=0;countrow<10;countrow++){
		temp[countrow*49] = Z2[countrow*49]*Wc[0];
		for(countcol=1;countcol<49;countcol++){
			temp[countrow*49 + countcol] = Z2[countrow*49 + countcol]*Wc[countcol];
		}
	}
